...
import type { SearchPredicate } from './SearchPredicate.js'

const RX_HIVE: RegExp = /\/|\\/
const RX_REPLACE: RegExp = /\//gi
...
export class RegKeys {
  ...
  get(forceRefresh: boolean = false): string[] {
    if (!isWindows()) {
      throw new Error(
        'This function only runs on the Windows operating system.'
      )
    }
 ...
 */
function extractHive(key: string): string {
  if (key && typeof key === 'string') {
    ...
    // magic 🔮😂
    const keys: string[] = key.split(RX_HIVE)
...
function expandHive(key: string): string {
  if (typeof key === 'string') {
    ...
    result = result.replace(RX_REPLACE, '\\')
